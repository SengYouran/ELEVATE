...
function Bags() {
  ...
  const { userAccount, currentAccount } = useDataContext();
    ...
    <div className="flex flex-col gap-4 cursor-pointer">
      ...
      <div className={`flex flex-col relative bg-gray-100 p-4`}>
        {bags.map((render) => {
          ...
          return (
            <div className="flex justify-between items-center mt-2" key={render?.id}>
              ...
              flex flex-col gap-1"
                >
                  ...
                  <p className="text-sm">Code. {render?.code}</p>
                  <p className="text-sm font-bold">{render?.cloth_colors?.[0]?.color} / {render?.sizes}</p>
              ...
              <div className="flex flex-col gap-1 text-right">
                ...
                {render?.discount && (
                  <p className="text-sm">
                    ({render?.discount} off) ${discountAmount.toFixed(2)}
                  </p>
                )}
