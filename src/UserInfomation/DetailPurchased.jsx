...
function DetailPurchased() {
  ...
  const orderId = queryParams.get("orderId"); // "320"
  const { setDetail, userAccount, currentAccount } = useDataContext();
  ...
  useEffect(() => {
    ...
    const getOneProduct = purchased?.find((check) => check.id == orderId);

    setDetailProduct(getOneProduct);
    setAddress(shippingAddress);
    setBank(bank);
  }, [userAccount, currentAccount, orderId]);
        ...
        <div className="w-full md:w-[60%] border pt-1 px-2">
          ...
          <div className="mt-2 relative">
            <div className="flex gap-2">
              <img
                className="w-30 h-30"
                src={detailProduct?.cloth_colors?.[0].product_image?.[1]}
                alt="Product Image Detial"
              />
